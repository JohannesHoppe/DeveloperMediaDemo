...
JQuery does a great job by hiding the browsers differences. It just works!™  
The capture phase was skipped due to to its lack of support in IE. It also simplifies the API. In the end, jQuery strips everything except bubbling (technical term: event propagation)
...
Browser events bubble from the innermost element (the event target) where they occur all the way up to the document element. The live() method waited on the document element for events, which was very slow! Now [delegated events](http://api.jquery.com/on/#direct-and-delegated-events) are recommended.
