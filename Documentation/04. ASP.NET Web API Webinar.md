...
<!--
2.4.1 [Simple Solution - URI](#level2uri)  
2.4.2 [Dirty Solution - POST](#level2post)    
2.4.3 [Better Solution - PATCH](#level2patch) 
-->
...
In Web API v1 there were two ways of creating a response from an API action. Either returning a specific object instance (the Web API pipeline converts it automatically to a default HttpResponseMessage) or returning a raw HttpResponseMessage. ASP.NET Web API 2 introduces IHttpActionResult. It is effectively a factory for HttpResponseMessage and by implementing the interface you provide instructions on how a new response should be constructed ([example](http://www.strathweb.com/2013/06/ihttpactionresult-new-way-of-creating-responses-in-asp-net-web-api-2/)).

Simple cases with method parameters and own return types (e.g. our Note) were never tricky to unit test. This snippet shows is a typical example:
...
public class When_requesting_notes
{
    ...
    Establish context = () =>
    {
        ...
        NoteController subject = new NoteController(repository);
    };
...
```
 ...
 But tests against a special request or configuration were a bit more complicated. But now the internal code behind new IHttpActionResult interface offers more convenience. ASP.NET Web API 2 makes it very easy to mock every part of an API controllers. The usual HttpResponseMessage result type can be directly replaced with that interface. The rest of the productive code can be left untouched. But test are now very easy to set up! Here here some examples! (driven by Machine.Specifications)
