...
In my opinion there is a golden mean between a pure CSS-based framework (like Bootstrap) combined with a MVVM framework (like Knockout) and jQuery mobile. This framework is called Kendo UI mobile. On the fist view, Kendo UI Mobil looks similar to jQuery mobile. It uses HTML5 data attributes, too. But it also has MVVM support build in. Please take a look at the **"CordovaKendoUiMobileDemo"** sample app. 
